...
void InitializeSdkObjects(FbxManager*& pManager, FbxScene*& pScene, const char* pFilename)
{
	...
	bool pImportStatus = pImporter->Initialize(pFilename, -1, pManager->GetIOSettings());
	...
	//int pFileMajor, pFileMinor, pFileRevision;

	//// Populate the FBX file format version numbers with the import file.
	//pImporter->GetFileVersion(pFileMajor, pFileMinor, pFileRevision);

	if( !pScene )
    {
        FBXSDK_printf("Error: Unable to import FBX scene!\n");
		system("pause");
        exit(1);
    }
}
