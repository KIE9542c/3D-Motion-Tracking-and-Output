...
std::vector<QString> vfInputPath;
...
void vfFile::mousePressEvent(QMouseEvent *event)
{
	//当鼠标左键点击时.
...
void vfFile::mouseMoveEvent(QMouseEvent *event)
{
...
void vfFile::mouseReleaseEvent(QMouseEvent *event)
{
	if (event->button() == Qt::LeftButton)
	{
		//改变移动状态.
...
void vfFile::on_pushButton_openfile_clicked()
{
	...
	if (!fileName.isEmpty())
	{
		vfInputPath.push_back(fileName);

		QFileInfo info(fileName);
		std::string s = fileName.toStdString();
		vfmodel->setItem(row, 0, new QStandardItem(info.fileName()));
		vfmodel->setItem(row, 1, new QStandardItem(info.absolutePath()));
		QString filesize;
		if (info.size() < 1024)
		...
		vfmodel->setItem(row, 2, new QStandardItem(filesize));
		...
		vfmodel->setItem(row, 3, new QStandardItem(QString::number(totalSecond / 60) + ":" + QString::number(totalSecond - totalSecond / 60 * 60)));
		row++;
		if (!vfInputPath.empty())
...
void vfFile::on_pushButton_del_clicked()
{
    ...
    while (r.hasPrevious()) {
	   ...
	   std::vector<QString>::iterator fbxtemp = vfInputPath.begin();
	   *fbxtemp += r.key();
	   vfInputPath.erase(fbxtemp);
    }
	if (vfmodel->rowCount() == 0)
	{
		ui->pushButton_del->setDisabled(true);
		ui->pushButton_delAll->setDisabled(true);
	}
