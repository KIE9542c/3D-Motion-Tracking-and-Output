...
int Darknet::get_int_from_cfg(std::map<string, string> block, string key, int default_value)
{
...
string Darknet::get_string_from_cfg(std::map<string, string> block, string key, string default_value)
{
	if ( block.find(key) != block.end() ) 
	{
		return block.at(key);
...
std::map<string, string>* Darknet::get_net_info()
{
	if (blocks.size() > 0)
	{
		return &blocks[0];
...
void Darknet::load_weights(const char *weight_file)
{
	...
	{
		std::map<string, string> module_info = blocks[i + 1];
...
void Darknet::load_pt(const char* weight_file)
{
	torch::serialize::InputArchive archive;
	archive.load_from(std::forward<string>(weight_file));
	this->load(archive);
}
...
torch::Tensor Darknet::write_results(torch::Tensor prediction, int num_classes, float confidence, float nms_conf)
{
    ...
    for (int i = 0; i < batch_size; i++)
    {
        ...
        auto cls_mask = image_prediction_data * (image_prediction_data.select(1, 6) == 0).to(torch::kFloat32).unsqueeze(1);
