...
def read_openpose_json(now_str, idx, subdir, smooth=True, *args):
    ...
    for file_name in json_files:
        ...
        if int(frame_indx[0]) <= 0:
            # 因为按照原样注册最初的帧，所以直接指定INDEX
            _tmp_data = data["people"][idx]["pose_keypoints_2d"]
        else:
            # 采用最接近前一帧的人物数据
            past_xy = cache[int(frame_indx[0]) - 1]

            # 如果取得了数据，就产生与INDEX的数量相对应的索引值的排列。如果不能取得的话，首先就确保INDEX的分量
...
def main(_):
    ...
    # 日付+indexディレクトリ作成
    subdir = '{0}/{1}_3d'.format(os.path.dirname(openpose_output_dir), os.path.basename(openpose_output_dir))
    os.makedirs(subdir)
